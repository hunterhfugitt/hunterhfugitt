...
//The big changes to make were to assign the variables appropiate values, as described in the comments. The code runs by pulling up an html file that reads the javscript. The javascript pulls the data from the csv file and stores it so that you can 
//Easily work with the data and manipulate it appropiately 
  ...
  .then(data => {  //ARROW function - pass data as a parameter into this anonymous function
    ...
    data.forEach(d => { //ARROW function - for each object in the array, pass it as a parameter to this function
		...
		let tokens = d.start.split("-");
  		...
  		let cx = mapFromDomainToRange(1, 365, 0, 1000, d.daysFromYrStart); //******* don't use 0, use mapFromDomainToRange. Hint- use 365 days and don't worry about leap years... it is ok
  		let cy = getYearPosition(d.year, gapBetweenTimelines, startOfTimelinesY); //******* don't use 0, use getYearPosition
  		let radius = mapFromDomainToRange(1, 160, 1, 160, d.cost); //******* don't use 10, use mapFromDomainToRange.  The max cost is 160, and the min cost is 1
